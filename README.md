...
目前 policy-only 神經網路不使用 values；保留這個欄位主要是為了資料分析、追溯勝負分布，或未來若要重新做局面勝率實驗時使用。
...
若要從 MySQL 棋譜資料訓練 policy network，可先輸出資料集：
...
```powershell
python src\train_policy.py --input out\policy_dataset.npz --output out\policy_model.pt --batch-size 256
```

啟動瀏覽器 API 時若 `out\policy_model.pt` 存在，AI 對弈頁面會自動顯示 policy 候選手；alpha-beta 搜尋會用 policy 分數做走法排序。
...
```powershell
python src\csa_browser_api.py --source mysql --db-host 140.135.65.53 --db-port 3306 --db-user 11211213 --db-name DB11211213 --policy-model out\policy_model.pt --policy-order-ply 2 --opening-book-ply 30 --opening-book-min-count 2
```
