"""
- 若已呼叫 load_nnue_model()，evaluate_position() 會改用 NNUEEvaluator。
- 若尚未載入 NNUE，evaluate_position() 會退回原本手工評估。
"""
...
from nnue_model import IncrementalNNUEState, NNUEEvaluator
...
_NNUE_EVALUATOR: NNUEEvaluator | None = None


def load_nnue_model(model_path: Path | str, device: str | None = None, cache_size: int = 100_000) -> NNUEEvaluator:
    """載入 NNUE 評估器，之後 evaluate_position() 會自動改用 NNUE。"""
    global _NNUE_EVALUATOR
    _NNUE_EVALUATOR = NNUEEvaluator(model_path, device=device, cache_size=cache_size)
    return _NNUE_EVALUATOR


def set_nnue_evaluator(evaluator: NNUEEvaluator | None) -> None:
    """直接設定 NNUE 評估器；測試或對弈腳本可用。"""
    global _NNUE_EVALUATOR
    _NNUE_EVALUATOR = evaluator


def clear_nnue_model() -> None:
    """清除 NNUE 評估器，evaluate_position() 會退回手工評估。"""
    global _NNUE_EVALUATOR
...
class SearchContext:
    ...
    nnue_state: IncrementalNNUEState | None = None
...
def evaluate_position_handcrafted(board: cshogi.Board) -> int:
...
def evaluate_position(board: cshogi.Board) -> int:
    """評估局面。若已載入 NNUE，使用 NNUE；否則使用原本手工評估。
    - 分數以centipawn-like整數表示。
    - 正分代表目前輪到下棋的一方較有利。
    - 負分代表目前輪到下棋的一方較不利。
    """
    if _NNUE_EVALUATOR is not None:
        return _NNUE_EVALUATOR.evaluate(board)
    return evaluate_position_handcrafted(board)
...
def evaluate_search_position(board: cshogi.Board, context: SearchContext) -> int:
    """優先使用與搜尋同步的增量累加器評估局面。"""
    if context.nnue_state is not None:
        assert _NNUE_EVALUATOR is not None
        context.nnue_state._check_board(board)
        return _NNUE_EVALUATOR.evaluate_state(context.nnue_state)
    return evaluate(board, context.evaluator)


def push_search_move(board: cshogi.Board, move: int, context: SearchContext) -> None:
    """同步推進棋盤與 NNUE 狀態。"""
    if context.nnue_state is not None:
        context.nnue_state.push(board, move)
    else:
        board.push(move)


def pop_search_move(board: cshogi.Board, context: SearchContext) -> None:
    """同步回復棋盤與 NNUE 狀態。"""
    if context.nnue_state is not None:
        context.nnue_state.pop(board)
    else:
        board.pop()
...
def quiescence(
    ...
    stand_pat = evaluate_search_position(board, context)
    ...
    for move in candidates:
        push_search_move(board, move, context)
        try:
            child_counts = dict(position_counts)
            child_key = repetition_key(board)
            child_counts[child_key] = child_counts.get(child_key, 0) + 1
            score = -quiescence(board, -beta, -alpha, child_counts, context, ply + 1)
        finally:
            pop_search_move(board, context)
        if score >= beta:
            return beta
        if score > alpha:
            alpha = score
    ...
    for move_index, move in enumerate(ordered_moves(board, legal_moves, context, ply, entry.best_move if entry else None)):
        ...
        try:
            ...
            if ply == 0 and context.root_move_evaluator is not None:
                score += context.root_move_evaluator(board)
        finally:
            pop_search_move(board, context)
        if score > best_score:
            best_score = score
...
def search_best_move(
    ...
    nnue_state = (
        _NNUE_EVALUATOR.create_state(board)
        if evaluator is None and _NNUE_EVALUATOR is not None and _NNUE_EVALUATOR.model_type == "halfkp"
        else None
    )
    best_score = _NNUE_EVALUATOR.evaluate_state(nnue_state) if nnue_state is not None else evaluate(board, evaluator)
    best_line = [best_move]
    completed_depth = 0
    total_nodes = 0
    timed_out = False
