...
if /I "%DATA_SOURCE%"=="mysql" (
  "%PYTHON_EXE%" src\csa_browser_api.py --host 127.0.0.1 --port 8000 --source mysql --db-host 140.135.65.53 --db-port 3306 --db-user 11211213 --db-password "%MYSQL_PASSWORD%" --db-name DB11211213 --policy-model "%POLICY_MODEL%" --policy-order-ply 2 --opening-book-ply 30 --opening-book-min-count 2
) else (
  "%PYTHON_EXE%" src\csa_browser_api.py --host 127.0.0.1 --port 8000 --source csa --policy-model "%POLICY_MODEL%" --policy-order-ply 2
)
